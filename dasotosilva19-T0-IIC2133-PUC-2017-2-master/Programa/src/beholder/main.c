...
void change_pixel_color(int row, int col, int color)
{
  /* Nos aseguramos de que se terminen los cambios en la imagen */
  cairo_surface_flush(image);

  /* Cambiamos el pixel al color correspondiente*/
  data[row*stride + col*4 + 0] = color;
  data[row*stride + col*4 + 1] = color;
  data[row*stride + col*4 + 2] = color;
  ...
  while(update_thread)
  {
    ...
    if(fscanf(stdin, "%s", command))
    {
      if(!strcmp(command, "END"))
      {
			...
			{
				if(!scanf("%d %d %d", &row, &col, &alive)) break;
        if (alive == 1)
				{
          change_pixel_color(row, col, 255);
        }
        else
				{
          change_pixel_color(row, col, 0);
        }
			}
...
void init_image(int height, int width)
{
  ...
  for(int j = 0; j < height; j++)
  {
    ...
    {
			// TODO: DELETE THIS AND SET ALL TO 0
			int color = (i+j)%2 ? 255 : 0;

			/* Blue channel */
      data[j*stride + i*4 + 0] = color;
			/* Green channel */
      data[j*stride + i*4 + 1] = color;
			/* Red channel */
      data[j*stride + i*4 + 2] = color;
			/* Alpha channel */
      data[j*stride + i*4 + 3] = color;
    }
  }
